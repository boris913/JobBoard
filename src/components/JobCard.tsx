...
export function JobCard({ job, view }: JobCardProps) {
  ...
  // Créer un identifiant simple pour la page détail (slug à partir du titre + entreprise)
  const jobId = encodeURIComponent(`${job.title}-${job.company}-${job.location}`);

  if (view === "grid") {
    return (
      <Link href={`/job/${jobId}`} className="group block">
        <div className="h-full rounded-xl border border-stone-200 bg-white/80 p-5 transition-all duration-200 hover:shadow-md hover:-translate-y-0.5">
          <h3 className="font-display text-lg font-semibold text-ink line-clamp-2">{job.title}</h3>
  ...
  // Vue liste
  return (
    <div className="group rounded-xl border border-stone-200 bg-white/80 p-5 transition-all duration-200 hover:shadow-md">
